                ...
                if errored_at == None or (how_long_error >= alert_term and monitoring_now == 1):
                    ...
                    if sqlcursor.rowcount > 0:
                        ...
                        for row in res:
                            ...
                            selectedTime	= row[0]
                            selectedDb		= row[1]
                            selectedCommand = row[2]
                            selectedInfo	= row[3]
                            selectedUser	= row[4]
                            selectedHost	= row[5]
                            selectedCnt	    = row[6]

                            selectedInfo = format_sql(selectedInfo)

                            if len(selectedInfo) > 7000:
                                selectedInfo = textwrap.shorten(selectedInfo, width=7000, placeholder='\n...(over 7,000 bytes)')
                            
                            attachment2['text'] = "Sample TIME = {0}\nSample DB = {1}\nSample COMMAND = {2}\nSample USER@HOST = {3}@{4}\nPattern COUNT = {5}\n```{6}```".format(selectedTime,
                                                                                                                                                                                selectedDb,
                                                                                                                                                                                selectedCommand,
                                                                                                                                                                                selectedUser,
                                                                                                                                                                                selectedHost,
