        ...
        for row in res:
            ...
            if checkTime < threshold_num:
                ...
                if errored_at != None:
                    ...
                    username = "Moni"
                    channel = "monitor"
                    text = "[OK] {0} {1}=*{2}*".format(dbname, curl_title, str(checkTime))
                    ...
                    sql = '''
                        ...
                        '''.format('Test', monitor_id, server_list_seqno, monitoring_code_seqno, checkTime, threshold_num, 'CURLOPT_POSTFIELDS', alert_term, check_count_threshold, check_count_current, monitoring_schedule)
                    cursor.execute(sql)
                    conn.commit()
            ...
            # 모니터링 조건에 맞는 결과가 있는 경우
                ...
                if errored_at == None or (how_long_error >= alert_term and monitoring_now == 1):
                    ...
                    if check_count_current >= check_count_threshold:
                        ...
                        username = "Moni"
                        channel = "monitor"
                        text = "[PROBLEM] {0} {1}=*{2}*".format(dbname, curl_title, str(checkTime))
